...
description: The best audio processing library built on Apple's MLX framework, providing fast and efficient text-to-speech (TTS), speech-to-text (STT), and speech-to-speech (STS) on Apple Silicon, as OpenAI Compatible API server.
...
The best audio processing library built on Apple's MLX framework, providing fast and efficient text-to-speech (TTS), speech-to-text (STT), and speech-to-speech (STS) on Apple Silicon, run as OpenAI Compatible API server.
...
```
This script will:
- clone (forked) mlx-audio repo to `~/opt/mlx-audio`
...
# output will be transcript text
...
NOTE:
- First runs will be a bit slow, since the server will need to download models (a few GBs) from Hugging Face.
